...
import os
import hashlib
from gen_uri import gen_uri
from sparql import sparql
from gen_graph import gen_graph


'''
classe permettant un stockage facile des paramètres reçus depuis la ligne de commande.
'''
class Params(object):
...
## tous les paramametres sont obligatoires
def get_params(argv):
	mots_clefs = ''
	...
	try:
		...
		print('$ main.py -m "mots clefs" -r 10 -s 1 -c 0.1 -f false -s 20 -a true')
		sys.exit(2)
	for opt, arg in opts:
		...
		if opt in ('-t', '--search_type'):
			search_type = gen_uri.SearchType(int(arg))
	...
	if max_number_of_results < 1 or max_number_of_results > 100:
		...
		print('$ main.py -m "mots clefs" -r 10 -s 1 -c 0.1 -f false -s 20 -a true')
		sys.exit(4)
	if not search_type in [s for s in gen_uri.SearchType]:
...
def main(argv):
	...
	if request_cached in os.listdir(cache_dir):
		...
		json_uris = gen_uri.main(parametres_main.mots_clefs
		...
		output = gen_graph.main(json_rdfs)
...
## exemple d'appel : 
## main.py --mots_clefs="mots clefs" --max_number_of_results=10 --search_type=1 --spotlight_confidence=0.1 --from_web=false --spotlight_support=20 --append_keyword=true
