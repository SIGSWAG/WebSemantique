...
var verbs = ["Calculating", "Computing", "Analyzing", "Generating",
	"Extrapolating", "Exploring", "Improving", "Processing", "Hashing",
	"Defining", "Quantifying", "Structuring", "Counting", "Measuring",
	"Abstracting", "Adding", "Substracting", "Multiplying", "Dividing",
	"Simplifying"
];
var adjs = ["exponential", "underlying", "differential", "overloaded",
	"underloaded", "integrated", "abstract", "rigorous", "related", "similar",
	"logic", "formal", "natural", "virtual", "alternative", "derivable",
	"dedicated", "included", "excluded", "imported", "exported"
];
var nouns = ["matrix", "values", "data", "search engines", "indices",
	"key/value pairs", "RDF triples", "numbers", "quantities", "structures",
	"patterns", "measures", "conclusion", "abstraction", "systems", "metadata",
...
$(function() {
	...
	var newLoading = function() {
		var minTimer = 1;
		var maxTimer = 10;
		var timer = (Math.floor(Math.random() * (maxTimer - minTimer)) + minTimer) *
			1000;
		var color = getRandomElement(colors);
		var $bar = $('<div class="progress-bar progress-bar-' + color + '"></div>')
	...
	var createRanking = function(json, num) {
		...
		rank = replaceAll(rank, "{{coef}}", (Math.floor(json.coeff * 100 * 1000) /
			1000));
		var $rank = $("<li>" + rank + "</li>");
		return $rank;
	};
	...
	var createFilm = function(json) {
		...
		if (json.infos) {
			...
			if (json.infos.director) {
				...
				film = replaceAll(film,
					'<a target="_blank" href="{{directorLink}}">{{directorName}}</a>',
					'{{directorName}}');
			}
			...
			if (json.infos.dirName) {
				...
				film = replaceAll(film, '{{directorName}}',
					'<span class="directorName">Unknown</span>');
			}

			if (json.infos.country) {
				film = replaceAll(film, "{{country}}", json.infos.country.value);
			} else {
				film = replaceAll(film, '{{country}}',
			...
			if (json.infos.starring) {
				if (json.infos.starring.type == "literal") {
					...
					var linkStarring = '<a target="_blank" href="' + uri + '">' + starring +
						'</a>';
				...
				film = replaceAll(film, "{{starring}}",
					'<span class="starring">Unknown</span>');
			}
		}
		var $film = $(film);
	...
	var createResult = function(json) {
		...
		for (var i = 0; i < json.results.films.length; i++) {
			...
			$result.find(".ranking").append(createRanking(json.results.films[i], i +
				1))
		};
		return $result;
	};
	...
	var loadResults = function(json) {
		Elements.$results.append($(
			'<h1 class="center" id="results-title">Results, and related films</h1>'
		));
		for (var i = 0; i < json.length; i++) {
	...
	var loadListFilm = function(json) {
		Elements.$filmList.append($(
			'<h1 class="center" id="list-title">Movies related to your search</h1>'
		));
		for (var i = 0; i < json.length; i++) {
			// Pour chaque resultats
	...
	$("#search").submit(function(e) {
		...
		params1.append_keyword = $("input[name='appendKeyword']:checked").length ?
			"true" : "false";

		States.loading();
		if (LOCAL) {
			setTimeout(function() {
				States.displayResults(jsonsample)
			}, 2 * 1000);
			setTimeout(function() {
				States.displayFilmList(jsonlistsample)
			}, 1 * 1000);
		} else {
			$.ajax({
				method: "GET",
